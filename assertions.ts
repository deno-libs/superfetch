import * as assert from 'https://deno.land/std@0.106.0/node/assert.ts'

const MAX_SHORT_BODY_LENGTH = 80

async function getBody(response: Response, ctx: AssertionContext) {
  if (ctx.body === undefined) ctx.body = await response.text()
...
export class BodyAssertion implements Assertion {
  ...
  async execute(actual: any, expected: any, response: Response, context: AssertionContext) {
    ...
    if (typeof this._expectedBody === 'string') {
      ...
      actual.body = await getBody(response, context)
      expected.body = regex.exec(actual.body) ? actual.body : `a body with a value that matches ${regex}`
